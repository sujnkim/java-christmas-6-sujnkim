...
### 플래너 시작 문구 출력
- [ ] OutputView: 정해진 문구를 출력한다
  - 문구 - ```안녕하세요! 우테코 식당 12월 이벤트 플래너입니다.```
---

### 방문 날짜 입력
- [ ] 사용자에게 방문 날짜를 입력받는다
    - [ ] InputView: 1 이상 31 이하의 숫자를 입력받는다
...
### 주문 메뉴 입력
- [ ] 사용자에게 주문 메뉴를 입력받는다
    - [ ] InputView: {메뉴이름}-{수량} 형태의 메뉴를 한번에 여러개 입력받는다
      - 문구 - ```주문하실 메뉴와 개수를 알려 주세요. (e.g. 해산물파스타-2,레드와인-1,초코케이크-1)```
    - [ ] 개발 요청 예외 처리
      - 형식이 다른 경우 - [ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.
      - 없는 메뉴인 경우 - [ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.
      - 수량이 1 이상이 아닌 경우 - [ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.
      - 중복 메뉴가 있는 경우 - [ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.
    - [ ] 추가 예외 처리
      - 음료만 주문한 경우 - [ERROR] 음료만 주문 시, 주문할 수 없습니다. 음료 이외의 메뉴도 포함해서 다시 입력해주세요.
      - 총수량이 20을 초과한 경우 - [ERROR] 메뉴는 한 번에 총 20개까지만 주문할 수 있습니다. 개수를 조정하여 다시 입력해주세요.

---
### 날짜 문구 출력
- [ ] OutputView: 사용자가 입력한 날짜에 따른 문구를 출력한다
  - 문구 - ```12월 3일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!```

### 주문 메뉴 출력
- [ ] 주문한 모든 메뉴의 이름과 수량을 출력한다(출력 순서는 자유)
```text
<주문 메뉴>
티본스테이크 1개
바비큐립 1개
...
142,000원
```
- [ ] 모든 메뉴의 가격을 모두 더하는 기능

### 증정 메뉴 출력
- [ ] OutputView: 증정메뉴를 출력한다
...
-31,246원
```
- [ ] 고객이 입력한 날짜, 총주문 금액을 기반으로 적용 이벤트를 계산하는 기능
    - [ ] 날짜
...
---
- [ ] JDK17 버전에서 실행 가능해야 한다. JDK17에서 정상적으로 동작해야 한다
...
- [ ] JUnit5와 AssertJ를 이용하여 본인이 정리한 기능 목록이 정상 동작하는지 테스트코드로 확인한다
- [ ] else 예약어를 쓰지 않는다
